        ...
        <div class="form-col">
            <?php foreach ($elementsForfaitises as $element): ?>
                ...
                <input type="number" 
                       ...
                       value="<?= htmlspecialchars($element['quantite']) ?>" readonly>
                   <?php endforeach; ?>
        </div>
        <div class="btn-group" style="margin-top: 15px;">
            <button type="submit" class="btn btn-valider">Corriger</button>
            ...
            <tbody>
                <?php if (!empty($elementsHorsForfait)): ?>
                    <?php foreach ($elementsHorsForfait as $index => $element): ?>
                        <tr>
                            <td><input type="date" name="date_<?= $index + 1 ?>" value="<?= htmlspecialchars($element['date']) ?>" readonly></td>
                            <td><input type="text" name="libelle_<?= $index + 1 ?>" value="<?= htmlspecialchars($element['libelle']) ?>" readonly></td>
                            <td><input type="number" name="montant_<?= $index + 1 ?>" value="<?= htmlspecialchars($element['montant']) ?>" step="0.01" readonly></td>
                            <td>
...
<?php if (isset($moisASelectionner) && !empty($moisASelectionner)): ?>
    <div class="justificatifs-actions">
        <div class="nb-justificatifs">
            ...
            <input type="number" id="nbJustificatifs" name="nbJustificatifs" value="<?= htmlspecialchars($ficheFrais['nbJustificatifs'] ?? '0') ?>" readonly>
        </div>
        <div class="btn-group" style="margin-top: 15px;">
            <button type="submit" class="btn btn-valider">Valider</button>
            <button type="reset" class="btn btn-reinitialiser">Réinitialiser</button>
